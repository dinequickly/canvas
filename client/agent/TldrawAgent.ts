...
export class TldrawAgent {
	...
	async prompt(input: AgentInput, { nested = false }: { nested?: boolean } = {}) {
		...
		try {
			...
			this.updateDockRunFailure(request, e)
		...
		if (!scheduledRequest) {
			...
			this.requests.setIsPrompting(false)
			this.requests.setCancelFn(null)
			this.completeDockRun(request, nested)
			return
		}
	...
	cancel() {
		...
		if (activeRequest?.uiOrigin === 'dock' && activeRequest.requestId) {
			TldrawAgentApp.get(this.editor)?.dock.cancelRun(activeRequest.requestId)
		}
	...
	private completeDockRun(request: AgentRequest, nested: boolean) {
		if (nested || request.uiOrigin !== 'dock' || !request.requestId) return
		TldrawAgentApp.get(this.editor)?.dock.completeRun(request.requestId)
	}

	private updateDockRunFailure(request: AgentRequest, error: unknown) {
		if (request.uiOrigin !== 'dock' || !request.requestId) return
		const message =
			error instanceof Error ? error.message : typeof error === 'string' ? error : 'Agent request failed.'
		TldrawAgentApp.get(this.editor)?.dock.failRun(request.requestId, message)
	}
