...
export const TldrawAgentAppProvider = memo(function TldrawAgentAppProvider({
	...
	useEffect(() => {
		...
		;(window as any).navigation = instance.navigation
		;(window as any).orchestrator = instance.orchestration
		;(window as any).artifacts = instance.artifacts
		;(window as any).dock = instance.dock
		...
		return () => {
			...
			delete (window as any).artifacts
			delete (window as any).dock
		}
...
export function useAgent(): TldrawAgent {
	const app = useTldrawAgentApp()
	return useValue('agent', () => app.agents.getAgent() ?? app.agents.ensureAtLeastOneAgent(), [app])
}
...
export function useOrchestrationState() {
	const app = useTldrawAgentApp()
	return useValue('orchestration-state', () => app.orchestration.getState(), [app])
}
...
export function useNavigationState() {
	const app = useTldrawAgentApp()
	return useValue('cerebrus-navigation-state', () => app.navigation.getState(), [app])
}

export function useDockState() {
	const app = useTldrawAgentApp()
	return useValue('dock-state', () => app.dock.getState(), [app])
}
