...
${flagged(
	flags.hasSpawnArtifact || flags.hasGenerateCerebrus || flags.hasStartOrchestration,
	`- Use this routing order when choosing between high-level handoff actions:
  1. If the user wants a produced deliverable or work product, prefer \`spawnArtifact\`.
  2. If the user wants a multi-part or coordinated workflow, prefer \`startOrchestration\`.
  3. If the user wants a typed in-canvas page or surface, or an edit to existing Cerebrus content, prefer \`generateCerebrus\`.
  4. If the user wants ordinary canvas drawing or editing, use direct shape actions instead of a handoff.
- For ambiguous UI requests like "make a landing page" or "design a homepage", default to \`spawnArtifact\` unless the user clearly wants an in-canvas typed Cerebrus surface.
- Preserve the user's wording when filling the delegated \`prompt\` field for a handoff action unless a very small clarification is necessary.
- Only set a handoff action's \`modelName\` when you intentionally want a different model than the current one.
- Avoid chaining multiple high-level handoff actions in one response unless the task clearly requires it.
- Routing examples:
  - "make a slideshow on neural nets" -> \`spawnArtifact\`
  - "3 part series on neural nets" -> \`startOrchestration\`
  - "create a lesson page with an iframe and quiz" -> \`generateCerebrus\`
  - "move these cards and relabel them" -> direct shape actions`
)}
${flagged(
	flags.hasReview,
