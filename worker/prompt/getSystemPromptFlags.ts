...
export function getSystemPromptFlags(actions: AgentAction['_type'][], parts: PromptPart['type'][]) {
	return {
		...
		hasSpawnArtifact: actions.includes('spawnArtifact'),
		hasGenerateCerebrus: actions.includes('generateCerebrus'),
		hasStartOrchestration: actions.includes('startOrchestration'),
