...
// Spawn Artifact Action
export const SpawnArtifactAction = z
	.object({
		_type: z.literal('spawnArtifact'),
		intent: z.string(),
		prompt: z.string().min(1),
		artifactType: z
			.enum(['slideshow', 'web-design', 'react-component', 'document', 'code', 'prototype'])
			.optional(),
		modelName: z.string().refine(isValidModelName, 'Unsupported model.').optional(),
	})
	.meta({
		title: 'Spawn Artifact',
		description:
			'The AI creates a deliverable artifact such as a slideshow, prototype, web design, or React component preview by delegating to the artifact agent. Set modelName explicitly when a specific artifact model should be used.',
	})

export type SpawnArtifactAction = z.infer<typeof SpawnArtifactAction>

// Generate Cerebrus Action
export const GenerateCerebrusAction = z
	.object({
		_type: z.literal('generateCerebrus'),
		intent: z.string(),
		prompt: z.string().min(1),
		modelName: z.string().refine(isValidModelName, 'Unsupported model.').optional(),
	})
	.meta({
		title: 'Generate Cerebrus',
		description:
			'The AI delegates to the typed Cerebrus generator to create or patch in-canvas pages and surfaces. Preserve the user request faithfully in the delegated prompt unless a small clarification is necessary.',
	})

export type GenerateCerebrusAction = z.infer<typeof GenerateCerebrusAction>

// Start Orchestration Action
export const StartOrchestrationAction = z
	.object({
		_type: z.literal('startOrchestration'),
		intent: z.string(),
		prompt: z.string().min(1),
		workflowHint: z.enum(['auto', 'series', 'calc1-course']).optional(),
	})
	.meta({
		title: 'Start Orchestration',
		description:
			'The AI starts a coordinated multi-artifact orchestration workflow. Use this for series, courses, or other prompts that need multiple related outputs rather than a single artifact.',
	})

export type StartOrchestrationAction = z.infer<typeof StartOrchestrationAction>
